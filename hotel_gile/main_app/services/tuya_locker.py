...
from hotel_gile.main_app.services.tuya_connector.ticketencryptor import encrypt_AES_128, decrypt_AES_128
from hotel_gile.main_app.services.tuya_connector.openapi import TuyaOpenAPI
...
def create_tuya_password(reservation):
    ...
    if password_ticket['success']:
        ...
        password = reservation.get_locker_password
...
def delete_tuya_password(reservation):
    openapi = connect()
    response = openapi.delete(f"/v1.0/devices/{TUYA_DEVICE_ID}/door-lock/temp-passwords/{reservation.password_id}")
    if response['success']:
        reservation.locker_password_id = None
        reservation.generate_password = False
